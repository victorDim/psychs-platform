...
After migration `0012_evidence_snapshot_context` and worker grant provisioning, each worker-collected observation records the latest unexpired snapshot for up to 100 verified sources in that observation's tenant and project, selected before its provider request. Selection is ordered by source ID and explicitly marked truncated if more sources exist. Each reference preserves the snapshot ID, source ID, original byte hash, capture time, and body retention deadline. This versioned context is part of the observation's content hash and immutable record; snapshot deletion does not block evidence retention or erase the reference. Old observations and external service ingestion have null context and are not backfilled. Provider requests do not receive these snapshot bodies: the relationship is `available_at_collection_start`, not proof that the provider consumed or cited a specific snapshot. A retried provider request selects its own context at that attempt's start.
